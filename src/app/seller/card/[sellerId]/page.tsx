        ...
        <div ref={cardRef} className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          {data.badges.length > 0 && (
            <div className="px-6 py-4 border-b border-slate-100">
              <h3 className="text-sm font-semibold text-slate-700 mb-3">成就徽章</h3>
              <div className="flex flex-wrap gap-3">
                {data.badges.map((badge) => (
                  <div key={badge.key} className="flex items-center gap-2 bg-indigo-50 rounded-lg px-3 py-2">
                    <span className="text-xl">{badge.icon}</span>
                    <div>
                      <div className="text-sm font-medium text-indigo-700">{badge.name}</div>
