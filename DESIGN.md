...
### 整体调性
- 参考风格：Linear / Vercel / Raycast 现代科技产品
- 干净、精致、有呼吸感
- 不要传统后台管理系统的白底+灰色+蓝色按钮风格

### 配色方案
- 深色背景：`#0F172A`（slate-900）或 `#1E293B`（slate-800）
- 渐变高亮：蓝紫渐变 `from-blue-500 to-purple-500` 或 青绿渐变 `from-cyan-400 to-teal-500`
- 微光效果：使用 `shadow-lg shadow-blue-500/20` 等带透明度的阴影
- 卡片背景：半透明毛玻璃 `bg-slate-800/50 backdrop-blur-xl border border-slate-700/50`

### 动效设计
- 节点流动动画：时间线节点之间有数据传输的流动效果
- 活跃节点脉冲：当前节点有呼吸灯效果 `animate-pulse`
- 卡片悬浮：`hover:scale-[1.02] transition-transform duration-300`
- 渐变流动：使用 `bg-gradient-to-r` 配合 `animate-gradient` 实现流动效果
...
### 图标风格
- 使用科技感线条图标（Lucide Icons）
- 每个节点用不同的图标表示状态
- 图标带渐变色或发光效果

### 字体排版
- 数字使用等宽字体 `font-mono`
- 标题加粗有层次 `font-bold tracking-tight`
- 时间戳使用较小字号 + 次要颜色

### 卡片风格
- 毛玻璃效果：`backdrop-filter: blur(12px)`
- 微阴影：`shadow-lg shadow-black/10`
- 悬浮缩放：`hover:scale-[1.02]`
- 边框：半透明渐变边框 `border border-white/10`

## 设计禁忌
- 不要使用圆角过大的卡片（max 12px）— 保持专业方正感
- 不要在进度条使用渐变色块 — 用纯色 ■ 和空心 □，清晰即正义（注：科技感页面除外）
- 不要使用过细的字体（min font-weight 400）
- 进度条不要用横向线型，必须用方块节点（注：物流时间线页面使用垂直时间线）
- 不要做得像传统后台管理系统（白底+灰色+蓝色按钮）
